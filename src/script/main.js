...
const salesContainer = document.querySelector("#weeklySales")
console.log(salesContainer)

salesByWeek.forEach(salesObject => {
    let agentHeader = document.createElement("h1");
    let agentDiv = document.createElement("div");
    console.log(salesObject)
    agentHeader.textContent = `${salesObject.sales_agent.first_name} ${salesObject.sales_agent.last_name}`;
    agentDiv.appendChild(agentHeader)


    for (const vehicleInfo of Object.entries(salesObject.vehicle)) {
        const agentVehicle = document.createElement("div");
        agentVehicle.textContent = `${vehicleInfo[0]}: ${vehicleInfo[1]}`;
        agentDiv.appendChild(agentVehicle);
    }

    let grossProfit = document.createElement("h3") 
    grossProfit.textContent = `total: ${salesObject.gross_profit}`;
    agentDiv.appendChild(grossProfit) 
